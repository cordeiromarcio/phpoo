...
class Pessoa
{
    ...
    {
        if (empty(self::$conn)) {
            $ini = parse_ini_file('config/myapp.ini');
            $host  = $ini['host'];
            $name  = $ini['name'];
            $user  = $ini['user'];
            $pass  = $ini['pass'];
    ...
    public static function find($id)
    {
        ...
        /*-------------------Sem proteção ataque SQL Injection-----------------------------
        $result = $conn->query("SELECT * FROM pessoa WHERE id = '{$id}'");
        */

        // Com proteção ataque SQL Injection
    ...
    public static function save($pessoa)
    {
        ...
        if (empty($pessoa['id'])) {
            ...
            $sql = "INSERT INTO pessoa (id, nome, endereco, bairro, telefone, email, id_cidade) VALUES (:id, :nome, :endereco, :bairro, :telefone, :email, :id_cidade )";
        } else {
